...
program lpt
  ...
  real,allocatable :: phi(:,:,:),phi_large(:,:,:),idsp(:,:,:,:),djdm(:,:,:),spin0(:,:,:,:)
  real,allocatable :: theta(:,:),imass_info(:,:),djdm_h(:,:)
  ...
  do ihalo=1,nhalo
    ...
    ind(:,ihalo)=floor(hcat(ihalo)%q/real(ng_global)*real(ngrid))+1 ! indeces in phi !!!!!!!!!!!!
  enddo
 ...
 ! construct halo mass bins
  ...
  allocate(djdm(ngrid,ngrid,ngrid),spin0(3,ngrid,ngrid,ngrid))
  allocate(djdm_h(n_rsmall,nhalo),theta(n_rsmall,nhalo))
  imassbin=nmassbin
  i2(nmassbin)=nhalo
  i1(1)=1
  itemp=1
  ...
  do ir=1,n_rsmall
    ...
    call get_djdm(r_small(ir)) ! loop over mass bins

    do ihalo=1,nhalo
      !print*,ihalo,nhalo,ind(:,ihalo)
      !print*,ind(:,ihalo)
      !print*,spin(:,ind(1,ihalo),ind(2,ihalo),ind(3,ihalo))
      !stop
      !theta(1,ihalo)=ccc(hcat(ihalo)%jt,spin0(:,ind(1,ihalo),ind(2,ihalo),ind(3,ihalo)))
      !theta(2,ihalo)=ccc(hcat(ihalo)%jl,spin0(:,ind(1,ihalo),ind(2,ihalo),ind(3,ihalo)))
      !print*,hcat(ihalo)%je, spin0(:,ind(1,ihalo),ind(2,ihalo),ind(3,ihalo))
      theta(ir,ihalo)=ccc(hcat(ihalo)%je,spin0(:,ind(1,ihalo),ind(2,ihalo),ind(3,ihalo)))
    enddo


    print*,r_small(ir),sum(djdm*1d0)/ngrid/ngrid/ngrid
    do ihalo=1,nhalo
      djdm_h(ir,ihalo)=djdm(ind(1,ihalo),ind(2,ihalo),ind(3,ihalo))
    enddo
    !call system_clock(tt2,t_rate)
    !print*, '  elapsed time =',real(tt2-tt1)/t_rate,'secs';
  enddo
  open(11,file=output_name('djdm_h'),status='replace',access='stream')
    write(11) n_rsmall,nhalo,djdm_h(:,:),theta(:,:)
  close(11)
  print*,'stop'
  stop
...
contains
  ...
  subroutine get_djdm(r_small)
    ...
    call gaussian_fourier_filter(phi_k,r_small*0.9)
    call sfftw_execute(plan_ifft_fine)
    !print*,rho_f(1:3,1,1); stop
    phi(1:ngrid,1:ngrid,1:ngrid)=rho_f(:ngrid,:,:)
    call buffer_1layer(phi)
    !call gaussian_fourier_filter(phi_k,r_small*1.05)
    call gaussian_fourier_filter_k2(phi_k,r_small*0.9)
    call sfftw_execute(plan_ifft_fine)
    phi_large(1:ngrid,1:ngrid,1:ngrid)=rho_f(:ngrid,:,:)
    call buffer_1layer(phi_large)
    call spinfield !(phi,phi_large,spin)
    ...
    do j=1,ngrid
    do i=1,ngrid
      djdm(i,j,k)=acos(ccc(spin(:,i,j,k),spin0(:,i,j,k)))*180/pi
    enddo
    enddo
    enddo

    !open(11,file=output_name('djdm'),status='replace',access='stream')
    !write(11) djdm
    !close(11)
    !stop
  endsubroutine
